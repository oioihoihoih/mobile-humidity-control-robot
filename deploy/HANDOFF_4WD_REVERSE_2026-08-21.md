...
> 최신 주변기기 배선은
> [`HANDOFF_DISTRIBUTED_PERIPHERALS_2026-08-21.md`](HANDOFF_DISTRIBUTED_PERIPHERALS_2026-08-21.md)를
> 따릅니다. 아래 문서의 DHT22/HC-SR04 배선은 최신 분배 메모가 우선합니다.

## 이번 변경의 기준

- MotorUno의 M1·M3는 왼쪽, M2·M4는 오른쪽입니다.
- 기존 모터는 M1/M2, 새 N20 1:298 모터는 M3/M4에 연결합니다.
- 출동 명령 `0x11`은 네 바퀴 전진, 복귀 명령 `0x12`는 네 바퀴 후진입니다.
...
- HC-SR04 본체는 N20 쪽 후방을 보며 MotorUno ECHO D2/TRIG A1에 연결합니다.
- MotorUno가 후진 중 유효 거리 15cm 미만이면 네 모터를 로컬 정지하고,
  18cm 이상이 3회 연속이면 자동 재개합니다. STUCK_HIGH도 로컬 정지하며
  NO_ECHO·OUT_OF_RANGE는 진단만 남깁니다.
- 세 Uno는 같은 커밋으로 함께 업로드합니다. command 7/status 8 exact ACK가 없거나 구형 이동값 `1/2`가 들어오면 HOME 보정과 이동이 잠깁니다. 현재 이동값은 `0x11/0x12`입니다.
...
- Robot/protocol/closed-loop tests: 129/129
- SimulIDE circuit/HEX manifest: PASS
- SensorUno: flash 27,190B, SRAM 1,396B (공개 예제 설정, AVR core 1.8.8)
- MotorUno: flash 11,446B, SRAM 468B
- ActuatorUno: flash 15,452B, SRAM 554B
