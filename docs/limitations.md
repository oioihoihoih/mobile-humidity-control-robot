...
SensorUno는 ESP-01 AT/HTTP, RC522, DHT22, 경로 상태와 두 I2C slave 조율을 함께 담당하므로 Uno R3 자원 여유가 작습니다. 후방 HC-SR04를 MotorUno로 옮기고 HOME RFID를 보조 도착 수단으로 추가한 현재 로컬 빌드는 flash/SRAM `27,808B / 1,428B`이고 CI budget은 `29,000B / 1,500B`입니다. 같은 빌드의 MotorUno는 `11,346B / 464B`, ActuatorUno는 `12,534B / 532B`입니다.
...
3. **저속 트랙:** 고전력 액추에이터를 분리한 채 전·후진 선 추종, 축별 PWM 정합, HOME·구역 태그 판독률과 정지거리를 측정하고 HOME 마커 fallback을 확인합니다.
