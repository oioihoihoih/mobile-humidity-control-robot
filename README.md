...
- 자동차는 `SensorUno`, `MotorUno`, `ActuatorUno`의 3개 Uno R3로 역할을 분리합니다.
- 2륜 구동부는 하나로 이어진 검은 선을 추종하고, 주행 중 RFID로 중간·목표 구역을 확인합니다.
...
| Uno 운영 스케치 | CI 컴파일 게이트 제공 | SensorUno는 현재 플래시 사용률이 약 99%라 컴파일 성공과 용량 변화를 계속 확인해야 합니다. |
| 실물 하드웨어 | **벤치 검증만 완료** | 정지 상태의 보드 연결, 기본 통신과 안전 출력만 확인한 범위입니다. |
| 연속 라인·RFID 실차 왕복 | 미검증 | 2륜 차체의 속도, 회전, 카드 판독, 제동거리와 HOME 진입을 트랙에서 검증해야 합니다. |
...
flowchart LR
    ...
    MOTOR --> DRIVE[2륜 구동 + 라인센서]
    ACT --> OUTPUT[가습·제습 릴레이 + LCD]
```
